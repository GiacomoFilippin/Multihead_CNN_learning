...
with zipfile.ZipFile(zip_path, 'r') as zip_ref:
    ...
    for i, track in enumerate(track_list):
        try:
            ...
            print(f"track {track_n} processed, shape: {mel_spectrogram.shape}")
            mel_spectrogram_plot = make_mel_spectrogram_plot(spectrogram=mel_spectrogram, sr=sr)
            mel_spectrogram_plot.show()
            """# Aggiungi al dataset
            spectrogram_dataset[os.path.basename(track)] = mel_spectrogram  # Usa solo il nome del file come chiave
                """
